...
in
{
  ...
  testScript = ''
    ...
    with subtest("Execute nik5"):
      machine.succeed("sudo -u osmuser nik5 --fit route --padding 60 --add-layers route --vars route=${./route}/route.geojson --ppi 300 -a 4 '${pkgs.openstreetmap-carto}/mapnik.xml' /tmp/map.png >&2")
